...
Status: **ALL TESTS PASSED** — backend 73/73, frontend 19/19.
...
| `src/services/email-worker.service.test.ts` | 12 | Missing job, already-sent skip, job-id mismatch, concurrent-claim skip, reschedule on rate limits, send success, SMTP failure → failed status, crash safety (rate limiter / DB failure releases job back to failed for retry) |
| `src/services/email-sweeper.service.test.ts` | 7 | Orphan with no BullMQ job id marked failed, job id not live marked failed, live job skipped, past-due cutoff query, live-state search, empty sweep, mid-sweep claim race not counted |
